 ...
 *
 * See `DOCUMENTATION.md` in this folder for the harness contract this implements.
 */
...
export const createClaudeBackendRuntime = (dependencies = {}) => {
  ...
  const readModelCatalog = async () => {
    ...
    const catalog = options
      ...
      .map((option) => ({
        ...
        ...(clampText(option.description, 160) ? { description: clampText(option.description, 160) } : {}),
      }));
  ...
  const buildSessionFromInfo = (info, fallbackDirectory) => buildSession({
    ...
    ...(info.gitBranch ? { metadata: { gitBranch: info.gitBranch } } : {}),
  });
